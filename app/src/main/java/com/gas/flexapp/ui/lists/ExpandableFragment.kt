...
class ExpandableFragment : Fragment() {
    ...
    private lateinit var expandableListComponent: ExpandableListComponent
    ...
    override fun onCreateView(
        ...
        if (_view == null) {
            ...
            expandableListComponent = ComponentBuilder.buildView(contentViewModel.content, context) as ExpandableListComponent
            _view = expandableListComponent.view

            expandableListComponent.setOnItemListClickListener(object :
                ItemListClickListener<ExpandableListModel> {
                override fun onClick(position: Int, item: JsonObject, itemList: ExpandableListModel) {
            ...
            lifecycleScope.launch {
                listViewModel.getItems(expandableListComponent.sourceUri)
            }
            ...
            setFragmentResultListener("onUpdatedItem") { _, bundle ->
                ...
                if (position > -1) {
                    ...
                    expandableListComponent.updateItem(position, item)
                }
            }
        ...
        lifecycleScope.launch {
            listViewModel.items.observe(viewLifecycleOwner) {
                ...
                if (state == Lifecycle.State.RESUMED) {
                    expandableListComponent.loadItems(it)
                }
            }
        }
